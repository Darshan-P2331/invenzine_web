...
import { faTrash, faEdit, faChevronDown, faComment } from "@fortawesome/free-solid-svg-icons";
...
export class Dashboard extends Component {
    ...
    async getData(user) {
        const posts = await firestore.collection('News').where('aemail', '==', user).orderBy("date", "desc").limit(9)
    ...
    async getDataLater(lastViewed, user) {
        const posts = await firestore.collection('News').where('aemail', '==', user).orderBy("date", "desc").startAfter(lastViewed).limit(9)
        posts.onSnapshot(querySnapshot => {
            this.setState({
                lastViewed: querySnapshot.docs[querySnapshot.docs.length - 1]
            })
    ...
    componentDidMount() {
        firebase.auth().onAuthStateChanged((user) => {
            firestore.collection('News').where('aemail', '==', user.email).onSnapshot(querysnapshot => {
                this.setState({
                ...
                {this.state.boards.length > 0 ?
                            ...
                            <Col lg={4} md={6} className='p-0'>
                                <Card className='mb-5' style={{ width: '300px' }}>
                                    ...
                                    <Card.Footer style={{ justifyContent: 'space-between' }}>
                                        ...
                                        <Button style={{ borderRadius: '50%', padding: '10px 0', width: '50px', marginLeft: '2em' }} variant='outline-primary' onClick={() => this.setState({ show: true, id: board.key })}><FontAwesomeIcon icon={faComment} /> </Button>
                                    </Card.Footer>
                                </Card>
                            </Col>
                        ))}
                    ...
                    <div className='d-flex justify-content-center'>
                        <div className='d-flex flex-column'>
                            <h1 className='text-secondary' style={{ marginTop: '250%',marginBottom: "100%" }}>Empty</h1>
                        </div>
                    </div>
                }
                <Comments show={this.state.show} onHide={() => this.setState({ show: false })} id={this.state.id} super={false} />
            </Container>
        )
    }
}
